...
class Th(threading.Thread):
  ...
  def getinfo(self,token, host, port, teacher_id):
     print "test2"
     urladdress=""
     username=""
     flag="true"
     url1="/api/v3/projects/%d/repository/files?private_token=%s&&file_path=%s&&ref=master" %(teacher_id, token, "timu")
     url2="/api/v3/projects/%d/repository/files?private_token=%s&&file_path=%s&&ref=master" %(teacher_id, token, "email")
     try:
	con=httplib.HTTPConnection(host, port, timeout=30)
	con.request("GET", url1)
	result=con.getresponse()
	if result.status==200:
           urladdress=base64.b64decode(json.loads(result.read())["content"])
           #urladdress=urladdress.replace("\n", '')
        else:
           flag="false"
        print urladdress
        con.request("GET", url2)
        result=con.getresponse()
	if result.status==200:
           username=base64.b64decode(json.loads(result.read())["content"])
	...
	if con:
	  con.close() 
     return {"result":flag, "urladdress":urladdress, "username":username}
  ...
  def run(self):
    ...
    if len(s)>4096:
      self.send_data("the code size is too long, must less 4k")
         ...
         if(s=="serve-info"):
            print "test"
         #   jresult=self.getinfo(config["teacher_token"], config["gitlab_host"], config["gitlab_port"], config["teacher_id"])
            comm= 'find %s -type f -name "*.html"' %(config["web"])
            status, outpout= commands.getstatusoutput(comm)
            #print outpout.split('\n')
            jresult = {"urladdress":outpout.replace(config["web"], ''), "username":''}
            #print jresult
            self.send_data(json.dumps(jresult))
            return True
         ...
         if result.has_key("command"):
             timuurl=result["url"]
             name=result["name"]
             chaxun=result["command"]

             a=timuurl.split('/')
             path=name+'/'+ a[3]+'/'+a[4].replace('html', 'md')
             print path
             url1="/api/v3/projects/%d/repository/files?private_token=%s&&file_path=%s&&ref=master" %(101, "X1Ua_uPpyijshQX-gPDs", path)
             con=httplib.HTTPConnection(config["gitlab_host"], config["gitlab_port"], timeout=30)
	     con.request("GET", url1)
	     result=con.getresponse()
             #print "test"
	     if result.status==200:
                content=base64.b64decode(json.loads(result.read())["content"])
                #print json.loads(content.split('\n')[1])["answer"]
                alla=content.split('\n')
                length=len(alla)
                print json.loads(alla[length-1], encoding='utf-8')["answer"]
                fname=open(config["web"]+a[3]+'/'+a[4].replace('.html', '_answer.md'), 'r')
                b={"result":True, "studentanswer":json.loads(alla[length-1], encoding='utf-8')["answer"], "referenceanswer":fname.read()}
                print b
                self.send_data(json.dumps(b))
             else:
                self.send_data(json.dumps({"result":False}))
             return True
  ...
  def send_data(self, data):
   # print data
    ...
    data = '%s%s' % (token, data)
    try:
      self.con.setblocking(1)
      a=self.con.send(data)
      print "a:",a
    except Exception,e:
      print e
    return True
